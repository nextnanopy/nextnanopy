...
class InputAssistant:
    ...
    def if_block(
        self, if_variable, if_content, elif_variable=None, elif_content=None, else_content=None
    ):
        content = (
            self.rc["nif"]
            + f"(${if_variable})"
    ...
    def region_obelisk(self, base_x, base_y, base_z, top_x, top_y, top_z):
        kwargs = dict(
            base_x=base_x, base_y=base_y, base_z=base_z, top_x=top_x, top_y=top_y, top_z=top_z
        )
    ...
    def region_polygonal_prism(
        self, axes=("x", "y", "z"), vertexes=((10.5, 14.0),), height=(0, 10)
    ):
        point = self.point(axes[2], *height)
        blocks = [
            self.region_vertex(ax1=axes[0], ax2=axes[1], value1=v[0], value2=v[1]) for v in vertexes
        ]
    ...
    def region_integrate(
        self, label="", electron=False, hole=False, piezo=False, pyro=False, polarization=False
    ):
        kwargs = {
    ...
    def contacts_schottky(self, name, bias, barrier, steps=1):
        ...
        if steps == 1:
            ...
            return self.equal_block(
                "schottky", dict(name=name, bias=bias, barrier=barrier, steps=steps)
            )
    ...
    def classical_output_bandedges(
        self, averaged="yes", profiles=("Gamma", "electron_fermi_level")
    ):
    ...
    def currents_recombination_model(
        self, SRH="no", Auger="no", radiative="no", enable_generation="yes"
    ):
        inputs = dict(
            SRH=SRH, Auger=Auger, radiative=radiative, enable_generation=enable_generation
        )
        kwargs = {}
        for key, value in inputs.items():
            if value:
    ...
    def quantum_output_wavefunctions(
        self, max_num=5, amplitudes="no", probabilities="yes", all_k_points="yes"
    ):
        return self.equal_block(
    ...
    def output_section(
        self, name, dim=2, x=None, y=None, z=None, range_x=None, range_y=None, range_z=None
    ):
