...
def is_nn3_input_file(fullpath):
    with open(fullpath, 'r') as f:
        for line in f:
            if nnfmts['nextnano3']['input_pattern'] in line:
                return True
    return False
...
def is_nnp_input_file(fullpath):
    with open(fullpath, 'r') as f:
        for line in f:
            if nnfmts['nextnano++']['input_pattern'] in line:
                return True
    return False
...
def is_nn3_variable(text):
    return is_variable(text, var_char=nnfmts['nextnano3']['var_char'])


def is_nnp_variable(text):
    return is_variable(text, var_char=nnfmts['nextnano++']['var_char'])
...
class InputAssistant(object):
    ...
    def __init__(self):
        global text, cbk, sbk, lines_to_text, modify_lines_in_text, paragraph, text_to_lines, quote
        global preview
        self.text = text
        self.sbk = sbk
        self.cbk = cbk
