...
folder_input = nn.config.config[software]['inputdirectory'] # path to the input file set in nnpy config file, change if needed
...
for x in [str(x) for x in SweepValues]:
    ...
    transmission_13_at_fixed_energy_x = df.variables['1->3'].value[int((E_meV - input_file.variables['E_min'].value)/input_file.variables['delta_energy'].value) + 1]
    transmission_13_at_fixed_energy.append(transmission_13_at_fixed_energy_x)
    transmission_14_at_fixed_energy_x = df.variables['1->4'].value[int((E_meV - input_file.variables['E_min'].value)/input_file.variables['delta_energy'].value) + 1]
