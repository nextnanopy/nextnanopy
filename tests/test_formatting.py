...
class TestNameUnit(unittest.TestCase):
    def test_str_to_name_unit(self):
        ...
        self.assertEqual(
            str_to_name_unit("test [eV] another[eV]", init="[", end="]"), ("test", "eV")
        )
        self.assertEqual(str_to_name_unit("test eV] ", init="[", end="]"), ("test eV]", None))
        self.assertEqual(
            str_to_name_unit("test[eV]_weird", init="[", end="]", add_rest_to_name=False),
            ("test", "eV"),
        )
        self.assertEqual(
            str_to_name_unit("test[eV]_weird", init="[", end="]", add_rest_to_name=True),
            ("test_weird", "eV"),
        )
        self.assertEqual(
            str_to_name_unit("test [eV] _weird", init="[", end="]", add_rest_to_name=False),
    ...
    def test_str_to_name_unit_with_rest(self):
        ...
        self.assertEqual(str_to_name_unit_with_rest("test[", init="[", end="]"), ("test[", None))
        self.assertEqual(
            str_to_name_unit_with_rest("test[eV", init="[", end="]"), ("test[eV", None)
        )
        self.assertEqual(
            str_to_name_unit_with_rest("test [eV", init="[", end="]"), ("test [eV", None)
        )
        ...
        self.assertEqual(
            str_to_name_unit_with_rest("test [eV] another[eV]", init="[", end="]"),
            ("test another", "eV"),
        )
        self.assertEqual(
            str_to_name_unit_with_rest("test eV] ", init="[", end="]"), ("test eV]", None)
        ...
        self.assertEqual(
            str_to_name_unit_with_rest("test[eV]_weird", init="[", end="]"), ("test_weird", "eV")
        )
        self.assertEqual(
            str_to_name_unit_with_rest("test [eV] _weird", init="[", end="]"), ("test _weird", "eV")
        )
